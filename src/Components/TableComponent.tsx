...
const SaveBtn = ({ coinId }: { coinId: string }) => {
  const { isSaved, saveCoin, deleteCoin } = useContext(StorageContext);
  ...
  return (
    <button onClick={handleClick}>
      <svg
        className={`ml-1.5 w-[1.5rem] hover:fill-cyan ${isSaved(coinId) ? 'fill-cyan' : 'fill-gray-100'}`}
        width="30"
        height="30"
        viewBox="0 0 30 30"
...
const TableComponent = ({
  ...
  savedComponent?: boolean;
}) => {
  const { currency, gettingCyptoData } = useContext(CryptoContext);
